...
# -*- coding: utf-8 -*-
"""
Тест v4.0 с расширенными паттернами для 150+ запросов
"""
...
from enhanced_query_router_v4 import EnhancedQueryRouterV4, QueryType
...
def test_v4_extended():
    """Тестирует v4.0 с расширенными паттернами"""
    
    router = EnhancedQueryRouterV4()
    test_queries = get_extended_test_queries()
    
    print("🎯 ТЕСТ V4.0 С РАСШИРЕННЫМИ ПАТТЕРНАМИ")
    print("=" * 70)
    ...
    # Инициализируем все типы из тестовых запросов
    for query, expected_type, should_process in test_queries:
        if expected_type not in results_by_type:
            results_by_type[expected_type] = 0
            correct_by_type[expected_type] = 0
    ...
    if errors:
        ...
        print("🎉 ОШИБОК НЕТ! Все запросы классифицированы правильно!")
        print()
    ...
    for query_type in results_by_type:
...
def test_router_responses_sample():
    ...
    print("\n🔧 ТЕСТ ОТВЕТОВ РОУТЕРА V4.0 (ВЫБОРКА):")
    print("=" * 70)
...
if __name__ == "__main__":
    accuracy, errors = test_v4_extended()
    analyze_error_patterns(errors)
    test_router_responses_sample()
    
    print(f"\n🎯 ФИНАЛЬНАЯ ТОЧНОСТЬ V4.0: {accuracy:.1f}%")
    
    if accuracy >= 95:
        print("🏆 СИСТЕМА V4.0 РАБОТАЕТ ОТЛИЧНО!")
    elif accuracy >= 90:
        print("👍 СИСТЕМА V4.0 РАБОТАЕТ ХОРОШО!")
    else:
        print("📈 Нужно улучшить систему v4.0")
