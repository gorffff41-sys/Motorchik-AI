...
def generate_yandex_response(messages):
    ...
    if not model_loaded:
        return "❌ Модель не загружена. Попробуйте перезапустить приложение."
    
    try:
        # Увеличиваем таймаут для генерации до 30 секунд
        import signal
        
        def timeout_handler(signum, frame):
            raise TimeoutError("Превышено время ожидания генерации ответа (30 секунд)")
        ...
        try:
            ...
            signal.alarm(0)
            return "⏰ Превышено время ожидания генерации ответа (30 секунд). Попробуйте еще раз."
        except Exception as e:
            signal.alarm(0)
            raise e
...
if __name__ == '__main__':
    print("🚀 Запуск полной версии Yandex GPT Чат-Бота...")
